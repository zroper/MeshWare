...
##        self.buy_value_elements = ['price','shipping','frequency']
    ...
    def sub_dict(input_key = '', input_value = ''):
        ...
        i = 0
        for key in sub_dict_builder_dict.keys():
...
##        self.drill_task_button.pack()
        ...
        self.e5 = Entry(self.entryframe5,
                         width = 15)
        self.e5.insert(0, "mylist_2_15_2016")
...
##        submittedVP1 = self.e3.get()
    ...
    def clearTask_callback(self=0):
        ...
        tasks_tab_panedwindow = ttk.Panedwindow(buttonframe, orient = HORIZONTAL)
        tasks_tab_panedwindow.pack(fill = BOTH, expand = True)
...
##            print(pp_dict)
## property name buttons need the following variables when packed,
            
##            parent_name
##            self_name
##            property_name (currently f from taskbuttonspack)
##            entry_object
...
##    s.configure('TButton', background = 'gold')
    ...
    logo = PhotoImage(file = '/Users/zroper/Dropbox/MeshWare/logo_MWbadge_gold.gif').subsample(10, 10)

    headTexture = PhotoImage(file = '/Users/zroper/Dropbox/MeshWare/head_MWtexture.gif')#.subsample(10, 10)
    ...
    ttpw_subtaskpane = ttk.Frame(tasks_tab_panedwindow, relief = SUNKEN)
    tasks_tab_panedwindow.add(ttpw_subtaskpane, weight = 4)
##    ttpw_DEFCONpane = ttk.Frame(tasks_tab_panedwindow, relief = SUNKEN)
##    tasks_tab_panedwindow.add(ttpw_DEFCONpane, weight = 1)
...
##    next_month_cal_label.grid(fill = X)
    ...
    progress_tab_frame.pack(fill = X)
    ...
    ProgressbarPanes = ttk.Panedwindow(tab2, orient = HORIZONTAL)
    ProgressbarPanes.pack(fill = BOTH, expand = True)

    barsframe = ttk.Frame(ProgressbarPanes, width = 200, height = 200, relief = SUNKEN)
    statsframe1 = ttk.Frame(ProgressbarPanes, width = 50, height = 200, relief = SUNKEN)
    statsframe2 = ttk.Frame(ProgressbarPanes, width = 50, height = 200, relief = SUNKEN)
    ...
    for i in range(5):
        progressbar = ttk.Progressbar(barsframe, orient = HORIZONTAL, length = 200)
        progressbar.config(mode = 'determinate', maximum = 100.0, value = i*25)
        progressbar.pack(fill = BOTH)
        ttk.Button(statsframe1, text = '-').pack(fill = X)
        ttk.Button(statsframe2, text = '+').pack(fill = X)
    ...
    ttkcal = Calendar(tab4, firstweekday=calendar.SUNDAY)
    ttkcal.pack()
